...
with open('CrossTransformer/data/predict.retrain_cross_transformer_one.every.feature.json','r') as f:
    every = json.load(f)
...
for data in quac_train['data']:
    for qas in data['paragraphs'][0]['qas']:
        label.append(' '.join(split_token(qas["orig_answer"]["text"])))
...
new_label = []
new_start = []
for e,p,l in zip(every, passages,label):
    e = [item["predict_text"] for item in e]
    an2text = answer_to_text(e,p)
    max_f1_answer = find_lcsubstr(an2text, l)
    if l != "CANNOTANSWER" and max_f1_answer == '':
        an2text = an2text + ' . ' + l
    context.append(an2text)

    if l == "CANNOTANSWER":       #CANNOTANSWER的判断
        ...
        if max_f1_answer == '':
            new_label.append(l)
            new_start.append(an2text.index(l))  
            num1 += 1
        else:
            new_label.append(max_f1_answer)
            new_start.append(an2text.index(max_f1_answer))
            num += 1
...
with open('quac/quac.dev.answer.transformer.one.json','w') as f:
